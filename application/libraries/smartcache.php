...
class smartcache {
    ...
    public function CacheName($name, $varyBy)
    {
        $divider = '_';

        if(array_key_exists('lang', $varyBy))
        {
            $name .= $divider.'lang~'.$varyBy['lang'];
        }

        if (array_key_exists('ajax', $varyBy) && $varyBy['ajax']) 
        {
            $name .= $divider.'ajax';
        }

        if (array_key_exists('category', $varyBy) && $varyBy['category']) 
        {
