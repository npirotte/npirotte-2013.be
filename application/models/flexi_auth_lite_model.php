...
class Flexi_auth_lite_model extends CI_Model
{
	...
	public function delete_database_login_session($user_id, $all_sessions = TRUE)
	{
		...
		{				
			...
			$sql_where = '('.
				'usess_uacc_fk = '.$this->db->escape($user_id).' AND '.
				'usess_series = '.$this->db->escape($this->hash_cookie_token($remember_series)).' AND '.
				'usess_token = '.$this->db->escape($this->hash_cookie_token($remember_token)).
			')';
			...
			if ($session_token = $this->auth->session_data[$this->auth->session_name['login_session_token']])
			{
				$sql_where = '('.
					'usess_uacc_fk = '.$this->db->escape($user_id).' AND '.
					'usess_token = '.$this->db->escape($session_token).
				')';
