...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	public function insert_user($email, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
		...
		if (!$this->identity_available($email, null, TRUE))
		{
		...
		if ($this->auth->primary_identity_col == $this->auth->tbl_col_user_account['username'] && !$this->identity_available($username, null, TRUE)) 
		{
		...
		else if (!empty($username) && !$this->username_available($username, null, TRUE))
		{

			if ($this->auth->auth_settings['auto_increment_username'])
			{
				$check_username = $username;
				for($i = 0; !$this->username_available($check_username, null, TRUE); $i++)
				{
	 ...
	 * @author Rob Hussey
	 */
	public function identity_available($identity = FALSE, $user_id = FALSE, $strict = FALSE)
	{		
	    if (empty($identity))
	    {
			return FALSE;
