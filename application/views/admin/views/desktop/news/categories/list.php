<div id="viewTools">
	<a class="btn btn-primary" href="#/carte/edit/new">Ajouter <i class="fa fa-plus-circle"></i></a>
</div>
	      ...
	      <div>
	          ...
	          <table class="table table-bordered table-striped table-hover" ng-show="items.length > 0">
	          	<thead>
	          		...
	          		<th ng-repeat="col in table">{{col.title}}</th>
	          		<th width="76px"></th>
	          	</thead>
	          	<tbody>
	          		...
	          		<tr ng-dblclick="getView(item.id)" class="{{item.statut}}" ng-repeat="item in items | filter:query | orderBy:orderProp:reverse">
	          					...
	          					<td ng-repeat="col in table">{{item[col.param]}}</td>
